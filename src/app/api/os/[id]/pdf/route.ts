...
export async function GET(
  ...
  // Parse notes JSON — exibe senha e estado físico separadamente
  ...
  const physicalConditionHtml = notesData.physicalCondition
    ? notesData.physicalCondition.split(',').map(i =>
  ...
  const passwordHtml = notesData.password
    ? `<tr>
        <td style="padding:8px 0;border-bottom:1px solid #f1f5f9;color:#64748b;font-size:0.85rem">Senha / Desbloqueio</td>
        <td style="padding:8px 0;border-bottom:1px solid #f1f5f9;text-align:right;font-weight:700;font-family:monospace">${notesData.password}</td>
       </tr>`
    : ''
...
<head>
<meta charset="UTF-8">
<title>OS #${osCode} – ${storeName}</title>
<style>
...
<body>
  ...
  <div class="header">
    ...
    <div style="text-align:right">
      ...
      <div style="font-size:1.2rem;font-weight:900;margin-top:6px">Nº: ${osCode}</div>
      <div style="font-size:0.75rem;color:#64748b;margin-top:4px">Entrada: ${fmtDate(os.createdAt)}</div>
      <div style="font-size:0.75rem;color:#64748b">Status: ${STATUS_MAP[os.status] || os.status}</div>
    </div>
  </div>
  ...
  <div class="grid2">
    <div class="section">
      ...
      <div class="field"><label>Nome</label><span>${os.customer.name}</span></div>
      ${os.customer.document ? `<div class="field"><label>Documento</label><span>${os.customer.document}</span></div>` : ''}
      ${os.customer.phone ? `<div class="field"><label>Telefone</label><span>${os.customer.phone}</span></div>` : ''}
      ${os.customer.email ? `<div class="field"><label>E-mail</label><span>${os.customer.email}</span></div>` : ''}
    </div>
    <div class="section">
      <div class="section-title">Dados do Aparelho</div>
      <div class="field"><label>Aparelho</label><span>${os.device} ${os.brand} ${os.model}</span></div>
      ${os.imei ? `<div class="field"><label>IMEI</label><span style="font-family:monospace">${os.imei}</span></div>` : ''}
      ${notesData.password ? `<div class="field"><label>Senha / Desbloqueio</label><span style="font-family:monospace">${notesData.password}</span></div>` : ''}
    </div>
  </div>
  ...
  <div class="section">
    ...
    <table>
      ...
      <tbody>
        <tr>
          <td style="padding:10px 12px;border-bottom:1px solid #f1f5f9">${os.defect}</td>
          <td style="padding:10px 12px;border-bottom:1px solid #f1f5f9;text-align:right;font-weight:700">${fmt(os.price)}</td>
        </tr>
      </tbody>
    </table>
    <div class="total-row">
