...
export default function OSClient({ serviceOrders, customers }: { serviceOrders: any[]; customers: any[] }) {
  ...
  const gerarRelatorio = () => {
    ...
    const rows = serviceOrders.map(o => `
      <tr style="border-bottom:1px solid #eee">
        ...
        <td style="padding:8px 6px;text-align:right">${fmtR(o.price || 0)}</td>
        <td style="padding:8px 6px">${fmtDate(o.createdAt)}</td>
      </tr>`).join('')
    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Relatório de OS</title>
      ...
      <div class="summary">
        <div class="stat"><div class="stat-label">Total de OS</div><div class="stat-value">${serviceOrders.length}</div></div>
        <div class="stat"><div class="stat-label">Em Bancada</div><div class="stat-value">${emBancada.length}</div></div>
        <div class="stat"><div class="stat-label">Valor Estimado</div><div class="stat-value">${fmtR(valorEstimado)}</div></div>
        <div class="stat"><div class="stat-label">Interrompidas</div><div class="stat-value">${interrompidas.length}</div></div>
      </div>
  ...
  const numberMap = useMemo(() => {
    const sorted = [...serviceOrders].sort(
  ...
  return (
    <>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <div>
            ...
            <p style={{ color: 'var(--color-text-muted)', fontSize: '0.85rem', marginTop: '2px' }}>
              Gestão de reparos, manutenções e aparelhos em bancada.
            </p>
          </div>
        </div>
                ...
                <tr>
                  <td colSpan={6}>
                    <div style={{ textAlign: 'center', padding: '64px 24px', color: 'var(--color-text-muted)' }}>
                      ...
                      <p style={{ fontSize: '1rem', fontWeight: '600', color: 'var(--color-text)', marginBottom: '8px' }}>Nenhuma ordem de serviço encontrada</p>
                      <p style={{ fontSize: '0.85rem' }}>Altere os filtros de busca ou crie uma nova OS.</p>
                    </div>
                  </td>
                </tr>
              )}
      ...
      {/* ── Modal: Nova OS (Multi-step Wizard) ── */}
      {isModalOpen && (
          ...
          <div style={{ backgroundColor: 'white', borderRadius: '16px', width: '100%', maxWidth: '800px', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.25)', overflow: 'hidden', display: 'flex', flexDirection: 'column', maxHeight: '90vh' }}>
            ...
            <div style={{ padding: modalStep === 7 ? '0' : '32px 48px', overflowY: 'auto', flex: 1, backgroundColor: modalStep === 7 ? '#f8fafc' : 'white' }}>
              <form action={handleAdd} id="os-form" style={{ height: modalStep === 7 ? '100%' : 'auto' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '20px' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                      <div>
                        ...
                        <input name="defect" defaultValue={osDevice.color ? `Cor: ${osDevice.color} - ` : ''} type="text" className="input" placeholder="Ex: Preto, Azul Metálico..." style={{ height: '48px' }} />
                        {/* Note: Saving color inside defect or notes. Alternatively you can handle this via FormData or schema. */}
                      </div>
                      <div>
                ...
                {/* STEP 3: Diagnóstico & Estado Físico */}
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                      ...
                      <div className="card" style={{ padding: '20px' }}>
                        ...
                        <input name="price" type="number" step="0.01" className="input" value={totalServicesPrice} readOnly style={{ height: '48px', fontSize: '1.2rem', fontWeight: '700', backgroundColor: '#f1f5f9' }} />
                        <p style={{ fontSize: '0.7rem', color: '#94a3b8', marginTop: '8px' }}>O valor será calculado automaticamente na aba de Serviços.</p>
                      </div>
                    </div>
                ...
                {/* STEP 4: Segurança do Dispositivo */}
                  ...
                  <div style={{ maxWidth: '400px', margin: '0 auto' }} className="card">
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                      ...
                      <div style={{ display: 'flex', backgroundColor: '#f1f5f9', borderRadius: '8px', padding: '4px' }}>
                        <button 
                          type="button" 
                          onClick={(e) => { e.preventDefault(); setLockType('padrao'); setDevicePassword(''); }}
                        ...
                        <button 
                          type="button" 
                          onClick={(e) => { e.preventDefault(); setLockType('desenho'); setDevicePassword(''); }}
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                        ...
                        {isAddingCustomService ? (
                          <div style={{ display: 'flex', gap: '8px', padding: '12px', border: '1px solid #cbd5e1', borderRadius: '8px', backgroundColor: '#f8fafc', alignItems: 'center' }}>
                            ...
                            }} style={{ backgroundColor: '#2563eb', color: 'white', border: 'none', borderRadius: '6px', padding: '0 12px', height: '36px', fontWeight: '600', cursor: 'pointer' }}>Salvar</button>
                            <button type="button" onClick={() => setIsAddingCustomService(false)} style={{ backgroundColor: 'transparent', color: '#64748b', border: 'none', cursor: 'pointer' }}><X size={18} /></button>
                          </div>
                        ) : (
                ...
                <div style={{ display: modalStep === 7 ? 'flex' : 'none', flexDirection: 'column', height: '100%' }}>
                  <div style={{ padding: '24px', display: 'flex', alignItems: 'center', gap: '12px', backgroundColor: 'white', borderBottom: '1px solid var(--color-border)' }}>
                    ...
                    <h2 style={{ fontSize: '1.2rem', fontWeight: '800', color: '#0f172a' }}>OS Criada</h2>
                  ...
                  <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '48px 24px' }}>
                    ...
                    <h3 style={{ fontSize: '1.8rem', fontWeight: '800', color: '#0f172a', marginBottom: '8px' }}>OS Criada com Sucesso!</h3>
                    <p style={{ color: '#64748b', fontSize: '0.95rem', marginBottom: '32px' }}>A ordem de serviço foi registrada.</p>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '24px' }}>
                      <button type="button" onClick={() => createdOs && window.open(`/api/os/${createdOs.id}/pdf`, '_blank')} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '28px 48px', backgroundColor: 'white', border: '2px solid #6366f1', borderRadius: '16px', cursor: 'pointer', gap: '12px', transition: 'all 0.2s', minWidth: '220px' }} className="hover-shadow">
                        ...
                        <div style={{ textAlign: 'center' }}>
                          <div style={{ fontSize: '1rem', fontWeight: '700', color: '#0f172a' }}>Imprimir / Salvar PDF</div>
                          <div style={{ fontSize: '0.75rem', color: '#64748b', marginTop: '4px' }}>Abre o documento A4 para imprimir ou salvar como PDF</div>
                        </div>
                      </button>
                    </div>
                    ...
                    <button 
                      ...
                      onClick={async () => {
                        if (!createdOs) return
                        const phone = createdOs.customer.phone || ''
                        if (!phone) { alert('Cliente sem telefone cadastrado'); return }
                        const result = await sendOsPdfWhatsApp(createdOs.id, phone)
                        if (result.error) alert('Erro: ' + result.error)
                        else alert('PDF enviado com sucesso pelo WhatsApp!')
                      }}
                      style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '20px', backgroundColor: 'white', border: '1px solid var(--color-border)', borderRadius: '12px', cursor: 'pointer', gap: '12px', transition: 'all 0.2s', width: '220px', marginBottom: '32px' }} className="hover-shadow">
                      ...
                      <div style={{ textAlign: 'center' }}>
                        ...
                        <div style={{ fontSize: '0.7rem', color: '#64748b', marginTop: '4px' }}>Envia o PDF da OS para o cliente</div>
                      </div>
                    </button>
                    
                    <div style={{ display: 'flex', gap: '16px' }}>
                      <button type="button" onClick={() => { setModalStep(1); setCreatedOs(null); setSelectedServiceIds(['1']); setPhysicalChecklist([]); setLockType('padrao'); setDevicePassword(''); }} style={{ padding: '10px 24px', fontSize: '0.9rem', fontWeight: '700', color: '#2563eb', background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px 24px', borderTop: '1px solid var(--color-border)', backgroundColor: 'white' }}>
              ...
              {modalStep < 6 ? (
                ...
                >
                  <CheckCircle size={18} style={{ marginRight: '6px' }} /> Finalizar OS
                </button>
              )}
            </div>
            )}
      ...
      {selectedOS && (
        <OSDetailsModal 
          os={selectedOS} 
          osNumber={numberMap[selectedOS.id] ?? 0} 
